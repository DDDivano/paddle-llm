...
np.random.seed(33)
input_data = generate_array(shape=[100352], dtype=np.float32, value_range=(-1, 1))
index = generate_array(shape=[100352], dtype=np.int64, value_range=(-1, 1))
updates = generate_array(shape=[100352], dtype=np.float32, value_range=(-1, 1))
overwrite = True
input_data_gradout = generate_array(shape=[100352], dtype=np.float32, value_range=(-1, 1))


def paddle_dynamic(dtype=np.float32, bf16=False):
    if dtype == np.float32:
        input = input_data.astype(np.float32)
        updates_ = paddle.to_tensor(updates.astype(np.float32))
        input_gradout = input_data_gradout.astype(np.float32)
    elif dtype == np.float16:
        input = input_data.astype(np.float16)
        updates_ = paddle.to_tensor(updates.astype(np.float16))
        input_gradout = input_data_gradout.astype(np.float16)
    else:
        updates_ = paddle.to_tensor(updates)
        input = input_data
    ...
    index_ = paddle.to_tensor(index)
    result = paddle.scatter(x, index_, updates_, overwrite)
...
def torch_dynamic(dtype=np.float32, bf16=False):
    if dtype == torch.float32:
        input = input_data.astype(np.float32)
        updates_ = torch.tensor(updates.astype(np.float32))
        input_gradout = input_data_gradout.astype(np.float32)
    elif dtype == torch.float16:
        input = input_data.astype(np.float16)
        updates_ = torch.tensor(updates.astype(np.float16))
        input_gradout = input_data_gradout.astype(np.float16)
    else:
        input = input_data
        updates_ = torch.tensor(updates)
        input_gradout = input_data_gradout
    if bf16:
    ...
    result = torch.scatter(x, dim=0, index=index_, src=updates_)
    # result.retain_grad()
...
def paddle_static(dtype=np.float32, bf16=False):
    ...
    x.stop_gradient = False
    index_ = paddle.to_tensor(index)
    result = paddle.jit.to_static(paddle.scatter)(x, index_, updates_, overwrite)
    grad = paddle.grad(result, x, grad_outputs=x_gradout)
    if bf16:
        result = paddle.cast(result, dtype="float32")
        grad = map_structure(lambda x: paddle.cast(x, dtype="float32"), grad)
