...
np.random.seed(33)
input_data = np.random.choice([True, False], size=(1, 1))
input_data_gradout = np.random.choice([True, False], size=(1, 1))
axis = None
keepdim = False
...
def paddle_dynamic(dtype=np.float32, bf16=False):
    ...
    result = paddle.any(x, axis, keepdim)
...
def paddle_static(dtype=np.float32, bf16=False):
    ...
    if bf16:
        ...
        # x_gradout = paddle.to_tensor(input_gradout)
    # x.stop_gradient = False
    result = paddle.jit.to_static(paddle.any)(x, axis, keepdim)
    # grad = paddle.grad(result, x, grad_outputs=x_gradout)
    if bf16:
        result = paddle.cast(result, dtype="float32")
        # grad = map_structure(lambda x: paddle.cast(x, dtype="float32"), grad)
...
def test_paddle_dynamic_vs_torch_bool():
    """
    paddle dynamic vs torch fp32
    :return:
    """
    # paddle_res, paddle_grad = paddle_dynamic(np.bool)
    # torch_res, torch_grad = torch_dynamic(np.bool)
    paddle_res = paddle_dynamic(np.bool)
    torch_res = torch_dynamic(np.bool)
...
def test_paddle_static_vs_torch_fp32():
    ...
    :return:
    """
    # paddle_res, paddle_grad = paddle_static(np.bool)
    # torch_res, torch_grad = torch_dynamic(np.bool)
    paddle_res = paddle_static(np.bool)
    torch_res = torch_dynamic(np.bool)
    Compare(paddle_res, torch_res, rtol=1e-6, atol=1e-6)
    # Compare(paddle_grad, torch_grad, rtol=1e-6, atol=1e-6)
...
def test_paddle_dynamic_stability_fp32():
    ...
    paddle_res = paddle_dynamic(np.bool)
    for i in range(5):
        # paddle_stability_res, paddle_stability_grad = paddle_dynamic(np.bool)
        paddle_stability_res = paddle_dynamic(np.bool)
...
def test_paddle_static_stability_fp32():
    ...
    paddle_res = paddle_static(np.bool)
    for i in range(5):
        # paddle_stability_res, paddle_stability_grad = paddle_static(np.bool)
        paddle_stability_res = paddle_static(np.bool)
        Compare(paddle_res, paddle_stability_res, rtol=1e-6, atol=1e-6)
        # Compare(paddle_grad, paddle_stability_grad, rtol=1e-6, atol=1e-6)
