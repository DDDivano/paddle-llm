...
np.random.seed(33)
input_data = generate_array(shape=[1, 4096, 1], dtype=np.float32, value_range=(-1, 1))
indices = generate_array(shape=[1, 2], dtype=np.int64, value_range=(1, 1000))
...
def paddle_dynamic(dtype=np.float32, bf16=False):
    ...
    result = paddle.gather_nd(x, paddle.to_tensor(indices))
...
def torch_dynamic(dtype=np.float32, bf16=False):
    ...
    result = torch.gather(x, torch.tensor(indices))
    result.retain_grad()
...
def paddle_static(dtype=np.float32, bf16=False):
    ...
    if bf16:
        ...
        x = paddle.to_tensor(input)
    x.stop_gradient = False
    result = paddle.jit.to_static(paddle.gather_nd)(x, paddle.to_tensor(indices))
    grad = paddle.grad(result, x)
    if bf16:
        result = paddle.cast(result, dtype="float32")
        grad = map_structure(lambda x: paddle.cast(x, dtype="float32"), grad)
